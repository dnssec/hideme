...
#include <windows.h>  // For opening URLs in Windows

void doPopAll(const std::string& first_name, const std::string& last_name) {
...
void doSearch(const std::string& site_key, const std::string& first_name, const std::string& last_name) {
    std::unordered_map<std::string, std::string> url_templates = {
        ...
        {"XLEK", "https://example.com/search/" + first_name + "+" + last_name} // Add your example URL here
    };

    auto it = url_templates.find(site_key);
    if (it != url_templates.end()) {
        std::string url = it->second;
        ShellExecute(NULL, "open", url.c_str(), NULL, NULL, SW_SHOWNORMAL);
    }
}

void doAll(const std::string& first_name, const std::string& last_name) {
    std::string site_keys[] = {
        "411info", "411", "Addresses", "Addresssearch", "AllPeople", "AdvancedBackgroundChecks", 
        "AdvancedPeopleSearch", "Anywho", "Arrestfacts", "Archives", "BackgroundAlerts", 
        "BackgroundCheckRun", "Blockshoppers", "Calltruth", "Centeda", "CheckPeople", "ClustrMaps", 
        "Classmates", "Cubib", "CyberBackgroundChecks", "Dataveria", "FamilyTreeNow", "FastPeople", 
        "FindPeopleSearch", "Freepeopledirectory", "IDcrawl", "Intelius", "Nuwber", "OfficialUSA", 
        "PeopleSearchNow", "PeoplebyName", "Radaris", "RocketReach", "SearchPeopleFree", 
        "Social-Searcher", "Spokeo", "Spytox", "ThatsThem", "TruePeople", "TruthFinder", 
        "Webmii", "WhitePages", "Yasni", "ZabaSearch", "XLEK" // Add your example site key here
    };

    for (const std::string& site_key : site_keys) {
        doSearch(site_key, first_name, last_name);
    }
}
...
int main() {
    ...
    doSearch("TruePeople", "John", "Doe");
    doSearch("FastPeople", "John", "Doe");
